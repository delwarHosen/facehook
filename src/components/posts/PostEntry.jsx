...
const PostEntry = ({onCreate}) => {
    ...
    const handlePostSubmit = async (formData) => {
        ...
        try {
            const respons = await api.post(`${import.meta.env.VITE_SERVER_BASE_URL}/posts`, formData);
            if (respons.status === 200) {
                dispatch({ type: actions.post.POST_CREATED, data: respons.data })
                onCreate()
            }

        } catch (error) {
            console.error(error);
            dispatch({ type: actions.post.DATA_FETCHED_ERROR, error: respons.error })
        }
    }
    ...
    return (
        <div className="card relative">
            ...
            <form
                ...
                >
                    <textarea
                        {...register("content", { required: "Adding some text is mendotiry!" })}
